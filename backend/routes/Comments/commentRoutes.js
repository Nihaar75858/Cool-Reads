...
router.get("/:blogId", authenticate, async (req, res) => {
  try {
...
router.post("/:id", authenticate, async (req, res) => {
  try {
    ...
    const { content, commenterId, recipientRole, commenterName } = req.body;

    // Validate required fields
    if (!content || !commenterId) {
      return res
        .status(400)
        .json({ message: "Content and commenterId are required" });
    }
    ...
    await Notification.create({
      type: "Comment",
      message: `${commenterName} just commented on your blog "${blog.title}"`,
      recipientRole,
